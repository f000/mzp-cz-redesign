 ...
 */
Modernizr.addTest("svgasimg",function(){return document.implementation.hasFeature("http://www.w3.org/TR/SVG11/feature#Image","1.1")});if(!Modernizr.svgasimg){$('img[src*="svg"]').attr("src",function(){return $(this).attr("src").replace(".svg",".png")})}function scrollToTop(sel){$(sel).click(function(){$("html, body").animate({scrollTop:0},"slow");return false})}function shuffleTagCloud(sel){var cloud=$(sel),tags=cloud.children();while(tags.length){cloud.append(tags.splice(Math.floor(Math.random()*tags.length),1)[0])}}function printPage(sel){$(sel).click(function(){window.print()})}function owlCarousel(sel){$(sel).owlCarousel({loop:false,margin:10,nav:true,dots:false,navText:['<span class="icon-lt2"></span>','<span class="icon-gt2"></span>'],responsive:{0:{items:3},600:{items:6},1e3:{items:8}}})}function titleTooltip(){$('#bg-1 abbr[title!=""], #bg-3 [title!=""]').tooltip({placement:"bottom",delay:1e3});$('#bg-5 [title!=""]').tooltip({placement:"top",delay:1e3});$("#bg-4 a").tooltip({placement:"bottom",delay:1e3,title:function(){return $(this).children("abbr").attr("title")},viewport:".owl-stage-outer"})}function colorboxLightbox(){$('a[rel^="lightbox"]').colorbox({rel:"gallery",opacity:.7})}function mainMenu(){$("#main-nav > ul > li > a").not("#main-nav > ul > li:first-child a").click(function(e){e.preventDefault()});$("#main-nav div").mouseleave(function(){$(this).delay(1e3).fadeOut()});$("#main-nav > ul > li").click(function(){$("#main-nav > ul > li").not($(this)).find("div").hide();$(this).children("div").toggle();var basicHeight=$(this).children("div").height();$(this).find("ul ul").each(function(){if($(this).height()<basicHeight){$(this).height(basicHeight+1)}})});$("#main-nav ul ul li").hover(function(){$(this).children("ul").toggle()})}function contactTelCollapsing(){var $main_list=$(".contactTel");var $triggers=$main_list.children("li:has(ul)").children("a");$triggers.siblings("ul").hide();$triggers.click(function(){var $node=$(this).parent();$node.siblings("li.act").andSelf().toggleClass("act").children("ul").toggle();return false})}function initMZP(){shuffleTagCloud(".tag-cloud");scrollToTop("#gotop a");printPage("#print a");owlCarousel(".owl-carousel");titleTooltip();colorboxLightbox();contactTelCollapsing();mainMenu();grunticon(["css/organizations/icons.data.svg.css","css/organizations/icons.data.png.css","css/organizations/icons.fallback.css"])}$(document).ready(initMZP);$(window).resize(function(){$(".carousel").trigger("refresh.owl.carousel")});